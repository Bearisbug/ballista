...
class TrackNet(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool(x1)                                               # (N,   64,  144,   256)
        x2 = self.down_block_2(x)                                       # (N,  128,  144,   256)
        x = self.pool(x2)                                               # (N,  128,   72,   128)
        x3 = self.down_block_3(x)                                       # (N,  256,   72,   128)
        x = self.pool(x3)                                               # (N,  256,   36,    64)
        x = self.bottleneck(x)                                          # (N,  512,   36,    64)
        x = torch.cat([self.up(x), x3], dim=1)                          # (N,  768,   72,   128)
        x = self.up_block_1(x)                                          # (N,  256,   72,   128)
        x = torch.cat([self.up(x), x2], dim=1)                          # (N,  384,  144,   256)
        x = self.up_block_2(x)                                          # (N,  128,  144,   256)
        x = torch.cat([self.up(x), x1], dim=1)                          # (N,  192,  288,   512)
